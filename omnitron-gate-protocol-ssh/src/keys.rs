...
fn get_keys_path(config: &OmnitronConfig) -> PathBuf {
  let mut path = config.paths_relative_to.clone();
  path.push(&config.store.ssh.keys);
  path
}

pub fn generate_host_keys(config: &OmnitronConfig) -> Result<()> {
  ...
  if !key_path.exists() {
    ...
    let key =
      PrivateKey::random(&mut get_crypto_rng(), russh::keys::Algorithm::Ed25519).context("Failed to generate Ed25519 key")?;
...
pub fn load_host_keys(config: &OmnitronConfig) -> Result<PrivateKey, russh::keys::Error> {
  let path = get_keys_path(config);
...
pub fn generate_client_keys(config: &OmnitronConfig) -> Result<()> {
  let path = get_keys_path(config);
  create_dir_all(&path)?;
  secure_directory(&path)?;
...
pub fn load_client_keys(config: &OmnitronConfig) -> Result<Vec<PrivateKey>, russh::keys::Error> {
  let path = get_keys_path(config);
  let mut keys = Vec::new();
