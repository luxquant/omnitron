...
use omnitron_gate_common::{SessionId, TargetSSHOptions};
...
impl russh::client::Handler for ClientHandler {
  ...
  async fn check_server_key(&mut self, server_public_key: &PublicKey) -> Result<bool, Self::Error> {
    ...
    match known_hosts
      .validate(&self.ssh_options.host, self.ssh_options.port, server_public_key)
      .await
    {
      Ok(KnownHostValidationResult::Valid) => Ok(true),
      Ok(KnownHostValidationResult::Invalid { key_type, key_base64 }) => {
        warn!(session=%self.session_id, "Host key is invalid!");
        return Err(ClientHandlerError::ConnectionError(ConnectionError::HostKeyMismatch {
      ...
      Ok(KnownHostValidationResult::Unknown) => {
        ...
        self
          .event_tx
          .send(ClientHandlerEvent::HostKeyUnknown(server_public_key.clone(), tx))
        ...
        if accepted {
          if let Err(error) = known_hosts
            .trust(&self.ssh_options.host, self.ssh_options.port, server_public_key)
            .await
          {
  ...
  async fn server_channel_open_x11(
    ...
    let _ = self
      .event_tx
      .send(ClientHandlerEvent::X11(channel, originator_address, originator_port));
    Ok(())
  }
}
