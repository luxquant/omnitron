...
use futures::pin_mut;
use omnitron_gate_common::{SSHTargetAuth, SessionId, TargetSSHOptions};
...
impl RemoteClient {
  ...
  fn set_state(&mut self, state: RCState) -> Result<(), SshClientError> {
    ...
    self.tx.send(RCEvent::State(state)).map_err(|_| SshClientError::MpscError)?;
    Ok(())
  }
  ...
  async fn apply_channel_op(&mut self, channel_id: Uuid, op: ChannelOperation) -> Result<(), SshClientError> {
      ...
      InnerEvent::ClientHandlerEvent(client_event) => {
        ...
        match client_event {
          ...
          ClientHandlerEvent::X11(channel, originator_address, originator_port) => {
            ...
            let id = self.setup_server_initiated_channel(channel).await?;
            let _ = self.tx.send(RCEvent::X11(id, originator_address, originator_port));
          }
          event => {
  ...
  async fn setup_server_initiated_channel(&mut self, channel: russh::Channel<russh::client::Msg>) -> Result<Uuid> {
    let id = Uuid::new_v4();
  ...
  async fn handle_command(&mut self, cmd: RCCommand) -> Result<bool, SshClientError> {
    match cmd {
      RCCommand::Connect(options) => match self.connect(options).await {
        Ok(_) => {
          self.set_state(RCState::Connected).map_err(SshClientError::other)?;
  ...
  async fn open_direct_tcpip(&mut self, channel_id: Uuid, params: DirectTCPIPParams) -> Result<(), SshClientError> {
    if let Some(session) = &self.session {
      let session = session.lock().await;
      let channel = session
      ...
      let channel = DirectTCPIPChannel::new(channel, channel_id, rx, self.tx.clone(), self.id);
      self.child_tasks.push(
        tokio::task::Builder::new()
          .name(&format!("SSH {} {:?} ops", self.id, channel_id))
          .spawn(channel.run())
  ...
  async fn cancel_tcpip_forward(&mut self, address: String, port: u32) -> Result<(), SshClientError> {
    if let Some(session) = &self.session {
      let session = session.lock().await;
