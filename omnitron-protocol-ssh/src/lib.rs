...
pub use common::*;
pub use keys::*;
use omnitron_common::{ListenEndpoint, ProtocolName, SshHostKeyVerificationMode, Target, TargetOptions};
...
impl ProtocolServer for SSHProtocolServer {
  ...
  async fn test_target(&self, target: Target) -> Result<(), TargetTestError> {
    ...
    let _ = handles.command_tx.send((RCCommand::Connect(ssh_options), None));
      ...
      match event {
        RCEvent::HostKeyUnknown(key, reply) => {
          println!(
            ...
            key
              .to_openssh()
              .map_err(|e| TargetTestError::ConnectionError(format!("ssh_key: {e:?}")))?
          );
        ...
        RCEvent::ConnectionError(err) => {
          ...
          } = err
          {
            ...
            println!("you can remove the old key in the Omnitron management UI and try again");
          }
        ...
        RCEvent::State(state) => match state {
          ...
          RCState::Disconnected => {
            return Err(TargetTestError::ConnectionError("Connection failed".to_owned()));
          }
          _ => {}
        },
        _ => {}
