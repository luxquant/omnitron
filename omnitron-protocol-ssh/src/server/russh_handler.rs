...
pub enum ServerHandlerEvent {
  ...
  AuthKeyboardInteractive(Secret<String>, Option<Secret<String>>, oneshot::Sender<Auth>),
...
impl ServerHandler {
  fn send_event(&self, event: ServerHandlerEvent) -> Result<(), ServerHandlerError> {
    self.event_tx.send(event).map_err(|_| ServerHandlerError::ChannelSend)
  }
}
...
impl russh::server::Handler for ServerHandler {
  ...
  async fn channel_open_session(&mut self, channel: Channel<Msg>, _session: &mut Session) -> Result<bool, Self::Error> {
    ...
    self.send_event(ServerHandlerEvent::ChannelOpenSession(ServerChannelId(channel.id()), tx))?;
  ...
  async fn subsystem_request(&mut self, channel: ChannelId, name: &str, session: &mut Session) -> Result<(), Self::Error> {
    ...
    self.send_event(ServerHandlerEvent::SubsystemRequest(ServerChannelId(channel), name, tx))?;
  ...
  async fn auth_publickey_offered(&mut self, user: &str, key: &russh::keys::PublicKey) -> Result<Auth, Self::Error> {
    ...
    self.send_event(ServerHandlerEvent::AuthPublicKeyOffer(user, key.clone(), tx))?;
  ...
  async fn auth_keyboard_interactive(
    ...
    self.send_event(ServerHandlerEvent::AuthKeyboardInteractive(user, response, tx))?;
  ...
  async fn signal(&mut self, channel: ChannelId, signal_name: russh::Sig, _session: &mut Session) -> Result<(), Self::Error> {
    let (tx, rx) = oneshot::channel();
    self.send_event(ServerHandlerEvent::Signal(ServerChannelId(channel), signal_name, tx))?;
    let _ = rx.await;
    Ok(())
  }
  ...
  async fn exec_request(&mut self, channel: ChannelId, data: &[u8], session: &mut Session) -> Result<(), Self::Error> {
    let data = Bytes::from(data.to_vec());
    let (tx, rx) = oneshot::channel();
    self.send_event(ServerHandlerEvent::ExecRequest(ServerChannelId(channel), data, tx))?;
