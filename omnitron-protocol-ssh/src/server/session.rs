...
use omnitron_common::auth::{AuthCredential, AuthResult, AuthSelector, AuthState, CredentialKind};
use omnitron_common::eventhub::{EventHub, EventSender, EventSubscription};
use omnitron_common::{OmnitronError, Secret, SessionId, SshHostKeyVerificationMode, Target, TargetOptions, TargetSSHOptions};
use omnitron_core::recordings::{
  self, ConnectionRecorder, TerminalRecorder, TerminalRecordingStreamId, TrafficConnectionParams, TrafficRecorder,
};
use omnitron_core::{authorize_ticket, consume_ticket, OmnitronServerHandle, Services};
...
use crate::{
  ChannelOperation, ConnectionError, DirectTCPIPParams, PtyRequest, RCCommand, RCCommandReply, RCEvent, RCState, RemoteClient,
  ServerChannelId, SshClientError, X11Request,
};
...
impl ServerSession {
  pub async fn start(
    ...
    let main_event_subscription = hub.subscribe(|e| !matches!(e, Event::ConsoleInput(_))).await;
    ...
    tokio::spawn(async move {
      loop {
        match so_rx.recv().await {
          Ok(data) => {
            if so_sender.send_once(Event::ServiceOutput(data)).await.is_err() {
              break;
            }
          }
  ...
  async fn get_auth_state(&mut self, username: &str) -> Result<Arc<Mutex<AuthState>>> {
    ...
    if self.auth_state.is_none() || self.auth_state.as_ref().unwrap().lock().await.username() != username {
      let state = self
        .services
        .auth_state_store
        .lock()
  ...
  fn map_channel(&self, ch: &ServerChannelId) -> Result<Uuid, OmnitronError> {
    self
      .channel_map
      .get_by_left(ch)
      .cloned()
      .ok_or(OmnitronError::InconsistentState)
  }
  ...
  pub async fn emit_service_message(&mut self, msg: &str) -> Result<()> {
    ...
    self.service_output.emit_output(Bytes::from(format!(
      ...
      Colour::Black.on(Colour::White).paint(" Omnitron "),
      msg.replace('\n', "\r\n"),
    )));
  ...
  pub async fn emit_pty_output(&mut self, data: &[u8]) -> Result<()> {
    ...
    for channel in channels {
      ...
      if let Some(session) = self.session_handle.clone() {
        self.channel_writer.write(session, channel.0, CryptoVec::from_slice(data));
      }
    }
    Ok(())
  }
  ...
  pub async fn maybe_connect_remote(&mut self) -> Result<()> {
    match self.target.clone() {
      ...
      TargetSelection::NotFound(name) => {
        self
          .emit_service_message(&format!("Selected target not found: {name}"))
          .await?;
  ...
  async fn connect_remote(&mut self, target: Target, ssh_options: TargetSSHOptions) -> Result<()> {
    self.rc_state = RCState::Connecting;
    self
      .send_command(RCCommand::Connect(ssh_options))
      .map_err(|_| anyhow::anyhow!("cannot send command"))?;
    self.service_output.show_progress();
    self
      .emit_service_message(&format!("Selected target: {}", target.name))
      .await?;

    Ok(())
  }

  fn handle_event<'a>(&'a mut self, event: Event) -> Pin<Box<dyn Future<Output = Result<(), OmnitronError>> + Send + 'a>> {
    async move {
      match event {
        Event::Client(RCEvent::Done) => Err(OmnitronError::SessionEnd)?,
        Event::ServerHandler(ServerHandlerEvent::Disconnect) => Err(OmnitronError::SessionEnd)?,
        Event::Client(e) => {
          debug!(event=?e, "Event");
  ...
  async fn handle_server_handler_event(&mut self, event: ServerHandlerEvent) -> Result<()> {
    match event {
      ...
      ServerHandlerEvent::SubsystemRequest(server_channel_id, name, reply) => {
        return match self._channel_subsystem_request(server_channel_id, name).await {
          Ok(()) => {
            let _ = reply.send(true);
            Ok(())
          }
  ...
  pub async fn handle_remote_event(&mut self, event: RCEvent) -> Result<()> {
    match event {
        ...
        match &self.rc_state {
          RCState::Connected => {
            ...
            self.service_output.emit_output(Bytes::from(format!(
              "{}{}\r\n",
              ERASE_PROGRESS_SPINNER,
              Colour::Black.on(Colour::Green).paint(" ✓ Omnitron connected ")
            )));
          }
      ...
      RCEvent::ConnectionError(error) => {
        ...
        match error {
          ConnectionError::HostKeyMismatch {
            ...
            self
              .emit_service_message("If you know that the key is correct (e.g. it has been changed),")
              .await?;
            self
              .emit_service_message("you can remove the old key in the Omnitron management UI and try again")
              .await?;
          }
          error => {
            self.service_output.emit_output(Bytes::from(format!(
      ...
      RCEvent::Close(channel) => {
        let server_channel_id = self.map_channel_reverse(&channel)?;
        let _ = self
          .maybe_with_session(|handle| async move { handle.close(server_channel_id.0).await.context("failed to close ch") })
          .await;
      }
      RCEvent::Eof(channel) => {
        let server_channel_id = self.map_channel_reverse(&channel)?;
        self
          .maybe_with_session(|handle| async move { handle.eof(server_channel_id.0).await.context("failed to send eof") })
          .await?;
      }
      ...
      RCEvent::ExitSignal {
        ...
        self
          .maybe_with_session(|handle| async move {
            handle
              .exit_signal_request(server_channel_id.0, signal_name, core_dumped, error_message, lang_tag)
              .await
              .context("failed to send exit status")?;
            Ok(())
          })
      ...
      RCEvent::HostKeyReceived(key) => {
        self
          .emit_service_message(&format!("Host key ({}): {}", key.algorithm(), key.public_key_base64()))
          .await?;
      }
      ...
      RCEvent::X11(id, originator_address, originator_port) => {
        if let Some(session) = &mut self.session_handle {
          let server_channel = session.channel_open_x11(originator_address, originator_port).await?;
  ...
  async fn handle_unknown_host_key(&mut self, key: PublicKey, reply: oneshot::Sender<bool>) -> Result<()> {
    self.service_output.hide_progress().await;

    let mode = self.services.config.lock().await.store.ssh.host_key_verification;
    ...
    self
      .emit_service_message(&format!("There is no trusted {} key for this host.", key.algorithm()))
      .await?;
  ...
  async fn _channel_open_direct_tcpip(&mut self, channel: ServerChannelId, params: DirectTCPIPParams) -> Result<bool> {
    ...
    match self
      .send_command_and_wait(RCCommand::Channel(uuid, ChannelOperation::OpenDirectTCPIP(params.clone())))
      .await
    {
      Ok(()) => {
  ...
  async fn _channel_exec_request(&mut self, server_channel_id: ServerChannelId, data: Bytes) -> Result<()> {
    let channel_id = self.map_channel(&server_channel_id)?;
  ...
  async fn _channel_x11_request(&mut self, server_channel_id: ServerChannelId, request: X11Request) -> Result<()> {
    ...
    let _ = self.maybe_connect_remote().await;
    self
      .send_command_and_wait(RCCommand::Channel(channel_id, ChannelOperation::RequestX11(request)))
      .await?;
    Ok(())
  }

  async fn _channel_env_request(&mut self, server_channel_id: ServerChannelId, name: String, value: String) -> Result<()> {
    let channel_id = self.map_channel(&server_channel_id)?;
  ...
  pub async fn _channel_subsystem_request(
    ...
    let _ = self.maybe_connect_remote().await;
    self
      .send_command_and_wait(RCCommand::Channel(channel_id, ChannelOperation::RequestSubsystem(name)))
      .await?;
    Ok(())
  }
  ...
  async fn _extended_data(&mut self, server_channel_id: ServerChannelId, code: u32, data: Bytes) -> Result<()> {
    let channel_id = self.map_channel(&server_channel_id)?;
    debug!(channel=%server_channel_id.0, ?data, "Data");
    let _ = self.send_command(RCCommand::Channel(
      channel_id,
  ...
  async fn _tcpip_forward(&mut self, address: String, port: u32) -> Result<()> {
    ...
    let _ = self.maybe_connect_remote().await;
    self
      .send_command_and_wait(RCCommand::ForwardTCPIP(address, port))
      .await
  ...
  pub async fn _cancel_tcpip_forward(&mut self, address: String, port: u32) -> Result<()> {
    ...
    self
      .send_command_and_wait(RCCommand::CancelTCPIPForward(address, port))
      .await
      .map_err(anyhow::Error::from)
  }

  async fn _auth_publickey_offer(&mut self, ssh_username: Secret<String>, key: PublicKey) -> russh::server::Auth {
  ...
  async fn _auth_publickey(&mut self, ssh_username: Secret<String>, key: PublicKey) -> russh::server::Auth {
    let selector: AuthSelector = ssh_username.expose_secret().into();
  ...
  async fn _auth_password(&mut self, ssh_username: Secret<String>, password: Secret<String>) -> russh::server::Auth {
    let selector: AuthSelector = ssh_username.expose_secret().into();
    info!("Password auth as {:?}", selector);

    match self.try_auth_lazy(&selector, Some(AuthCredential::Password(password))).await {
    ...
    match self.try_auth_lazy(&selector, cred).await {
      ...
      Ok(AuthResult::Need(kinds)) => {
        if kinds.contains(&CredentialKind::Totp) {
          ...
          let event = self.services.auth_state_store.lock().await.subscribe(auth_state_id);
          self.keyboard_interactive_state = KeyboardInteractiveState::WebAuthRequested(event);

          let mut login_url = match self.services.config.lock().await.construct_external_url(None, None) {
            Ok(url) => url,
            Err(error) => {
          ...
          login_url.set_path("@omnitron");
          login_url.set_fragment(Some(&format!("/login/{auth_state_id}")));

          russh::server::Auth::Partial {
            name: Cow::Borrowed("Omnitron authentication"),
            instructions: Cow::Owned(format!(
              concat!(
                "-----------------------------------------------------------------------\n",
                "Omnitron authentication: please open the following URL in your browser:\n",
                "{}\n\n",
  ...
  async fn try_validate_public_key_offer(&mut self, selector: &AuthSelector, credential: Option<AuthCredential>) -> Result<bool> {
    match selector {
      AuthSelector::User { username, .. } => {
        let cp = self.services.config_provider.clone();

        if let Some(credential) = credential {
          return Ok(cp.lock().await.validate_credential(username, &credential).await?);
        }
  ...
  async fn try_auth_lazy(&mut self, selector: &AuthSelector, credential: Option<AuthCredential>) -> Result<AuthResult> {
  ...
  async fn try_auth_eager(&mut self, selector: &AuthSelector, credential: Option<AuthCredential>) -> Result<AuthResult> {
    match selector {
      AuthSelector::User { username, target_name } => {
        ...
        if let Some(credential) = credential {
          if cp.lock().await.validate_credential(username, &credential).await? {
        ...
        match user_auth_result {
          AuthResult::Accepted { username } => {
            ...
            if !target_auth_result {
              warn!("Target {} not authorized for user {}", target_name, username);
      ...
      AuthSelector::Ticket { secret } => match authorize_ticket(&self.services.db, secret).await? {
        Some(ticket) => {
  ...
  async fn _auth_accept(&mut self, username: &str, target_name: &str) -> Result<(), OmnitronError> {
  ...
  async fn _channel_close(&mut self, server_channel_id: ServerChannelId) -> Result<()> {
    ...
    self
      .send_command_and_wait(RCCommand::Channel(channel_id, ChannelOperation::Close))
      .await?;
    Ok(())
  }
  ...
  pub async fn _channel_signal(&mut self, server_channel_id: ServerChannelId, signal: Sig) -> Result<()> {
    let channel_id = self.map_channel(&server_channel_id)?;
    debug!(channel=%channel_id, ?signal, "Signal");
    self
      .send_command_and_wait(RCCommand::Channel(channel_id, ChannelOperation::Signal(signal)))
      .await?;
    Ok(())
  }
  ...
  fn poll(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Self::Output> {
    match self.get_mut() {
      PendingCommand::Waiting(ref mut rx) => match Pin::new(rx).poll(cx) {
        Poll::Ready(result) => Poll::Ready(result.unwrap_or(Err(SshClientError::MpscError))),
