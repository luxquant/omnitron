...
use omnitron_gate_common::{configure_tls_connector, TargetPostgresOptions, TlsMode};
...
impl PostgresClient {
  pub async fn connect(target: &TargetPostgresOptions, options: ConnectionOptions) -> Result<Self, PostgresError> {
    let mut stream = PostgresStream::new(TcpStream::connect((target.host.clone(), target.port)).await?);

    if target.tls.mode != TlsMode::Disabled {
      stream.push(pgwire::messages::startup::SslRequest::new())?;
      stream.flush().await?;

      let Some(response) = stream.recv::<pgwire::messages::response::SslResponse>().await? else {
      ...
      if response == pgwire::messages::response::SslResponse::Accept {
        ...
        let client_config = Arc::new(configure_tls_connector(accept_invalid_certs, accept_invalid_hostname, None).await?);

        stream = stream
          .upgrade((
            target.host.clone().try_into().map_err(|_| PostgresError::InvalidDomainName)?,
            client_config,
          ))
          .await?;
    ...
    startup.parameters.insert("user".to_owned(), target.username.clone());
    ...
    loop {
      ...
      let get_password = || target.password.as_ref().ok_or(PostgresError::PasswordRequired);
      ...
      match payload.0 {
        ...
        PgWireBackendMessage::Authentication(auth) => match auth {
          ...
          pgwire::messages::startup::Authentication::CleartextPassword => {
            ...
            let password_message = pgwire::messages::startup::Password::new(password.into());
            stream.push(password_message)?;
            stream.flush().await?;
          }
          pgwire::messages::startup::Authentication::MD5Password(scramble) => {
            let password = get_password()?;
            let hashed = pgwire::api::auth::md5pass::hash_md5_password(&target.username, password, &scramble);
          ...
          pgwire::messages::startup::Authentication::SASL(mechanisms) => {
            let password = get_password()?;
            PostgresClient::run_sasl_auth(&mut stream, mechanisms, &target.username, password).await?;
          }
          x => {
        ...
        _ => {
          return Err(PostgresError::ProtocolError("Expected authentication".to_owned()));
        }
      }
    }
  ...
  async fn run_sasl_auth(
    ...
    while {
      ...
      match payload.0 {
        ...
        PgWireBackendMessage::Authentication(pgwire::messages::startup::Authentication::SASLContinue(msg)) => {
          data = Some(msg.to_vec());
        }
        PgWireBackendMessage::Authentication(pgwire::messages::startup::Authentication::SASLFinal(msg)) => {
          data = Some(msg.to_vec());
        }
        payload => {
          return Err(PostgresError::ProtocolError(format!("Unexpected message: {payload:?}",)));
        }
      }
    }
  ...
  pub async fn recv(&mut self) -> Result<Option<PgWireGenericBackendMessage>, PostgresError> {
    self.stream.recv::<PgWireGenericBackendMessage>().await.map_err(Into::into)
  }

  pub async fn send<M: PostgresEncode + Debug>(&mut self, message: M) -> Result<(), PostgresError> {
