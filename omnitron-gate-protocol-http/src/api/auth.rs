...
use omnitron_gate_common::auth::{AuthCredential, AuthResult, AuthState, CredentialKind};
use omnitron_gate_common::{OmnitronError, Secret};
...
use crate::common::{
  authorize_session, endpoint_auth, get_auth_state_for_request, RequestAuthorization, SessionAuthorization, SessionExt,
};
...
impl Api {
  ...
  async fn api_auth_login(
    ...
    let state_arc = match get_auth_state_for_request(&body.username, session, &mut auth_state_store).await {
      Err(OmnitronError::UserNotFound(_)) => {
    ...
    if cp.validate_credential(state.username(), &password_cred).await? {
    ...
    match state.verify() {
      ...
      x => {
        error!("Auth rejected");
        Ok(LoginResponse::Failure(Json(LoginFailureResponse { state: x.into() })))
      }
    }
  }
  ...
  async fn api_auth_otp_login(
    ...
    match state.verify() {
      ...
      x => Ok(LoginResponse::Failure(Json(LoginFailureResponse { state: x.into() }))),
    }
  }
  ...
  #[oai(path = "/auth/state", method = "get", operation_id = "getDefaultAuthState")]
  async fn api_default_auth_state(&self, session: &Session, services: Data<&Services>) -> poem::Result<AuthStateResponse> {
  ...
  #[oai(path = "/auth/state", method = "delete", operation_id = "cancelDefaultAuth")]
  async fn api_cancel_default_auth(&self, session: &Session, services: Data<&Services>) -> poem::Result<AuthStateResponse> {
    let Some(state_id) = session.get_auth_state_id() else {
      return Ok(AuthStateResponse::NotFound);
    };
...
async fn get_auth_state(id: &Uuid, services: &Services, auth: Option<&RequestAuthorization>) -> Option<Arc<Mutex<AuthState>>> {
  let store = services.auth_state_store.lock().await;
...
async fn serialize_auth_state_inner(state_arc: Arc<Mutex<AuthState>>, services: &Services) -> poem::Result<AuthStateResponse> {
  let state = state_arc.lock().await;
