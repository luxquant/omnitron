...
use gate_common::auth::{AuthCredential, CredentialKind, CredentialPolicy};
use gate_common::{OmnitronError, Secret, Target, User};
...
pub trait ConfigProvider {
  ...
  async fn authorize_target(&mut self, username: &str, target: &str) -> Result<bool, OmnitronError>;
...
pub async fn consume_ticket(db: &Arc<Mutex<DatabaseConnection>>, ticket_id: &Uuid) -> Result<(), OmnitronError> {
  let db = db.lock().await;
