...
use tracing::*;
use uuid::Uuid;
use omnitron_common::{ProtocolName, SessionId, Target, OmnitronError};
use omnitron_db_entities::Session;

use crate::{SessionHandle, OmnitronServerHandle};
...
impl State {
    ...
    pub async fn register_session(
    ...
    ) -> Result<Arc<Mutex<OmnitronServerHandle>>, OmnitronError> {
        ...
        match self.this.upgrade() {
            Some(this) => Ok(Arc::new(Mutex::new(OmnitronServerHandle::new(
                id,
                self.db.clone(),
                this,
                state,
