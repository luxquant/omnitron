...
use bytes::Bytes;
use omnitron_common::helpers::fs::secure_directory;
use omnitron_common::{OmnitronConfig, RecordingsConfig, SessionId};
...
impl SessionRecordings {
  pub fn new(db: Arc<Mutex<DatabaseConnection>>, config: &OmnitronConfig) -> Result<Self> {
  ...
  pub async fn remove<P: AsRef<Path>>(&self, session_id: &SessionId, name: P) -> Result<()> {
    ...
    if let Some(parent) = path.parent() {
      if tokio::fs::read_dir(parent).await?.next_entry().await?.is_none() {
