...
impl MigrationTrait for Migration {
  async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
    ...
    manager.create_table(schema.create_table_from_entity(role::Entity)).await?;
    manager.create_table(schema.create_table_from_entity(target::Entity)).await?;
    manager
  ...
  async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
    manager
      .drop_table(Table::drop().table(target_role_assignment::Entity).to_owned())
      .await?;
    manager.drop_table(Table::drop().table(target::Entity).to_owned()).await?;
    manager.drop_table(Table::drop().table(role::Entity).to_owned()).await?;
    Ok(())
  }
}
