...
use gate_common::{OmnitronError, User, UserPasswordCredential, UserRequireCredentialsPolicy};
use gate_core::Services;
use gate_db_entities::{self as entities, Parameters, PasswordCredential, PublicKeyCredential};
use http::StatusCode;
use poem::web::Data;
...
impl Api {
  ...
  async fn api_get_credentials_state(
  ...
  ) -> Result<CredentialsStateResponse, OmnitronError> {
    ...
    let otp_creds = user_model.find_related(entities::OtpCredential::Entity).all(&*db).await?;
  ...
  async fn api_change_password(
  ...
  ) -> Result<ChangePasswordResponse, OmnitronError> {
    let db = services.db.lock().await;
    ...
    let new_credential = entities::PasswordCredential::ActiveModel {
      ...
      ..PasswordCredential::ActiveModel::from(UserPasswordCredential::from_password(&body.password.clone().into()))
    }
    .insert(&*db)
    .await
    .map_err(OmnitronError::from)?;
  ...
  async fn api_create_pk(
  ...
  ) -> Result<CreatePublicKeyCredentialResponse, OmnitronError> {
    let db = services.db.lock().await;
    ...
    Ok(CreatePublicKeyCredentialResponse::Created(Json(object.into())))
  }
  ...
  async fn api_delete_pk(
    ...
    services: Data<&Services>,
    id: Path<Uuid>,
  ) -> Result<DeleteCredentialResponse, OmnitronError> {
    let db = services.db.lock().await;
  ...
  async fn api_create_otp(
  ...
  ) -> Result<CreateOtpCredentialResponse, OmnitronError> {
    let db = services.db.lock().await;
    ...
    entities::User::ActiveModel::try_from(user)?.update(&*db).await?;
