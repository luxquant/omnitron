...
from tests.conftest import OmnitronProcess
...
class Test:
    def test_basic(
        ...
        response = session.get(
            f"{url}/?omnitron-target={target.name}", allow_redirects=False
        )
        ...
        assert (
            unquote(redirect)
            == f"/@omnitron#/login?next=/?omnitron-target={target.name}"
        )

        response = session.get(f"{url}/@omnitron/api/info").json()
        assert response["username"] is None

        response = session.post(
            f"{url}/@omnitron/api/auth/login",
            json={
        ...
        response = session.get(
            f"{url}/some/path?a=b&omnitron-target={target.name}&c=d",
