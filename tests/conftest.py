...
binary_path = (
    "target/llvm-cov-target/debug/omnitron"
    if enable_coverage
    else "target/debug/omnitron"
)
...
class ProcessManager:
    ...
    def start_mysql_server(self):
        port = alloc_port()
        self.start(
            ["docker", "run", "--rm", "-p", f"{port}:3306", "omnitron-e2e-mysql-server"]
        )
        return port

    def start_postgres_server(self):
        port = alloc_port()
        container_name = f"omnitron-e2e-postgres-server-{uuid.uuid4()}"
        self.start(
            [
                "docker",
                "run",
    ...
    def start_wg(
        ...
        config="",
        args=None,
        share_with: Optional[OmnitronProcess] = None,
        stderr=None,
        stdout=None,
    ) -> OmnitronProcess:
        ...
        if share_with:
            ...
            config_path = data_dir / "omnitron.yaml"
        ...
        def run(args, env={}):
            return self.start(
                ...
                env={
                    **os.environ,
                    "LLVM_PROFILE_FILE": f"{cargo_root}/target/llvm-cov-target/omnitron-%m.profraw",
                    "OMNITRON_ADMIN_TOKEN": "token-value",
                    **env,
                },
        ...
        if not share_with:
            p = run(
                ...
                env={"OMNITRON_ADMIN_PASSWORD": "123"},
            )
            p.communicate()
