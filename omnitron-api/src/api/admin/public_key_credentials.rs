...
use gate_common::{OmnitronError, UserPublicKeyCredential};
use gate_db_entities::PublicKeyCredential;
use poem::web::Data;
...
impl TryFrom<&NewPublicKeyCredential> for UserPublicKeyCredential {
  ...
  fn try_from(credential: &NewPublicKeyCredential) -> Result<Self, OmnitronError> {
    let key = russh::keys::PublicKey::from_openssh(&credential.openssh_public_key).map_err(russh::keys::Error::from)?;
...
impl ListApi {
  ...
  ) -> Result<GetPublicKeyCredentialsResponse, OmnitronError> {
...
impl DetailApi {
  ...
  async fn api_update(
    ...
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<UpdatePublicKeyCredentialResponse, OmnitronError> {
    let db = db.lock().await;
    ...
    match model {
      Ok(model) => Ok(UpdatePublicKeyCredentialResponse::Updated(Json(model.into()))),
