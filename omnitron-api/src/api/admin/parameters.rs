...
use gate_db_entities::Parameters;
use poem::web::Data;
...
impl Api {
  ...
  async fn api_get(&self, services: Data<&Services>, _auth: AnySecurityScheme) -> Result<GetParametersResponse, OmnitronError> {
  ...
  #[oai(path = "/parameters", method = "patch", operation_id = "update_parameters")]
  async fn api_update_parameters(
    &self,
    services: Data<&Services>,
    body: Json<ParameterUpdate>,
    _auth: AnySecurityScheme,
  ) -> Result<UpdateParametersResponse, OmnitronError> {
    let db = services.db.lock().await;
