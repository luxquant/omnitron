...
use gate_common::{OmnitronError, Role as RoleConfig, Target as TargetConfig, TargetOptions};
use gate_core::consts::BUILTIN_ADMIN_ROLE_NAME;
use gate_db_entities::Target::TargetKind;
use gate_db_entities::{Role, Target, TargetRoleAssignment};
use poem::web::Data;
...
impl ListApi {
  ...
  async fn api_get_all_targets(
  ...
  ) -> Result<GetTargetsResponse, OmnitronError> {
    ...
    let targets = targets.all(&*db).await.map_err(OmnitronError::from)?;

    let targets: Result<Vec<TargetConfig>, _> = targets.into_iter().map(|t| t.try_into()).collect();
    let targets = targets.map_err(OmnitronError::from)?;
  ...
  async fn api_create_target(
    ...
    _auth: AnySecurityScheme,
  ) -> Result<CreateTargetResponse, OmnitronError> {
    ...
    let values = Target::ActiveModel {
      ...
      options: Set(serde_json::to_value(body.options.clone()).map_err(OmnitronError::from)?),
    };

    let target = values.insert(&*db).await.map_err(OmnitronError::from)?;

    Ok(CreateTargetResponse::Created(Json(
      target.try_into().map_err(OmnitronError::from)?,
    )))
  }
}
...
impl DetailApi {
  ...
  async fn api_get_target(
    ...
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<GetTargetResponse, OmnitronError> {
    let db = db.lock().await;
  ...
  async fn api_update_target(
    ...
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<UpdateTargetResponse, OmnitronError> {
    let db = db.lock().await;
    ...
    model.options = Set(serde_json::to_value(body.options.clone()).map_err(OmnitronError::from)?);
  ...
  #[oai(path = "/targets/:id", method = "delete", operation_id = "delete_target")]
  async fn api_delete_target(
    &self,
    db: Data<&Arc<Mutex<DatabaseConnection>>>,
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<DeleteTargetResponse, OmnitronError> {
    let db = db.lock().await;
...
#[OpenApi]
impl RolesApi {
  #[oai(path = "/targets/:id/roles", method = "get", operation_id = "get_target_roles")]
  async fn api_get_target_roles(
    &self,
    db: Data<&Arc<Mutex<DatabaseConnection>>>,
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<GetTargetRolesResponse, OmnitronError> {
    let db = db.lock().await;
  ...
  #[oai(path = "/targets/:id/roles/:role_id", method = "post", operation_id = "add_target_role")]
  async fn api_add_target_role(
    ...
    role_id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<AddTargetRoleResponse, OmnitronError> {
    let db = db.lock().await;
  ...
  async fn api_delete_target_role(
    ...
    role_id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<DeleteTargetRoleResponse, OmnitronError> {
    let db = db.lock().await;
