...
use gate_common::{OmnitronError, Secret, UserPasswordCredential};
use gate_db_entities::PasswordCredential;
use poem::web::Data;
...
impl ListApi {
  ...
  ) -> Result<GetPasswordCredentialsResponse, OmnitronError> {
  ...
  async fn api_create(
    ...
    user_id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<CreatePasswordCredentialResponse, OmnitronError> {
    let db = db.lock().await;

    let object = PasswordCredential::ActiveModel {
      id: Set(Uuid::new_v4()),
      user_id: Set(*user_id),
      ..PasswordCredential::ActiveModel::from(UserPasswordCredential::from_password(&body.password))
    }
    .insert(&*db)
    .await
    .map_err(OmnitronError::from)?;

    Ok(CreatePasswordCredentialResponse::Created(Json(object.into())))
  }
}
