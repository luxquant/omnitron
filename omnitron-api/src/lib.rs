...
use gate_common::{ListenEndpoint, Target, TargetOptions, TlsCertificateAndPrivateKey, TlsCertificateBundle, TlsPrivateKey};
...
#[async_trait]
impl ProtocolServer for HTTPApiServer {
  async fn run(self, address: ListenEndpoint) -> Result<()> {
    let admin_api_app = admin_api::admin_api_app(&self.services).into_endpoint();
    let api_service =
      OpenApiService::new(crate::api::user::get(), "Omnitron user API", env!("CARGO_PKG_VERSION")).server("/@omnitron/api");
    ...
    let cache_static = || SetHeader::new().overriding(http::header::CACHE_CONTROL, HeaderValue::from_static("max-age=86400"));

    let (cookie_max_age, session_max_age) = {
      let config = self.services.config.lock().await;
      (config.store.http.cookie_max_age, config.store.http.session_max_age)
    };
    ...
    let app = Route::new()
      .nest(
        ...
        Route::new()
          ...
          .nest_no_strip("/assets", EmbeddedFilesEndpoint::<Assets>::new().with(cache_static()))
          .nest(
            "/admin/api",
          ...
          .at(
            "/admin",
            page_auth(page_admin_auth(EmbeddedFileEndpoint::<Assets>::new("src/admin/index.html"))).with(cache_bust()),
          )
          .at(
            "",
    ...
    let certificate_and_key = {
      ...
      TlsCertificateAndPrivateKey {
        ...
        private_key: TlsPrivateKey::from_file(&key_path)
          .await
          .with_context(|| format!("reading TLS certificate from '{}'", certificate_path.display()))?,
      }
    };
