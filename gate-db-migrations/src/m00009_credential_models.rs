...
mod credential_enum {
  ...
  mod serde_base64_secret {
    ...
    mod serde_base64 {
      ...
      pub fn serialize<S: Serializer, B: AsRef<[u8]>>(bytes: B, serializer: S) -> Result<S::Ok, S::Error> {
        serializer.serialize_str(&BASE64.encode(bytes.as_ref()))
      }

      pub fn deserialize<'de, D: serde::Deserializer<'de>, B: From<Vec<u8>>>(deserializer: D) -> Result<B, D::Error> {
        let s = String::deserialize(deserializer)?;
        Ok(BASE64.decode(s.as_bytes()).map_err(serde::de::Error::custom)?.into())
      }
    }

    pub fn serialize<S: Serializer>(secret: &Vec<u8>, serializer: S) -> Result<S::Ok, S::Error> {
...
impl MigrationTrait for Migration {
  async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
    ...
    for user in users {
      ...
      let Ok(credentials) = serde_json::from_value::<Vec<UserAuthCredential>>(user.credentials.clone()) else {
        error!(
