...
use tabled::settings::{Color, Modify, Rotate, Width};
use tabled::{Table, Tabled};

use crate::helpers::{self, ColoredString};
use crate::process::{http, ItemSingle, Runner};
use crate::{config, file, log};
      ...
      if let Ok(json) = serde_json::to_string(&data[0]) {
        match format.as_str() {
          ...
          _ => {
            ...
            println!(
              " {}",
              format!("Use `omnitron logs {} [--lines <num>]` to display logs", self.id).white()
            );
            println!(
              " {}",
              format!("Use `omnitron env {}`  to display environment variables", self.id).white()
            );
          }
        };
      };
    ...
    if matches!(self.server_name, "internal" | "local") {
      if let Some(home) = home::home_dir() {
        ...
        if let Ok(process) = Process::new(item.pid as u32) {
          ...
          cpu_percent = Some(crate::service::get_process_cpu_usage_percentage(item.pid as i64));
        }
      ...
      let data: (crate::process::Process, Runner);
      let Some(servers) = config::servers().servers else {
        crashln!("{} Failed to read servers", *helpers::FAIL)
      };
  ...
  pub fn logs(mut self, lines: &usize) {
    if !matches!(self.server_name, "internal" | "local") {
      ...
      println!(
        "{}",
        format!(
          "Showing last {lines} lines for {}process [{}] (change the value with --lines option)",
          self.kind, self.id
        )
        .yellow()
      );
      ...
      if runner.is_empty() {
        ...
        for (id, item) in runner.items() {
          ...
          if internal {
            ...
            if let Ok(process) = Process::new(item.pid as u32) {
              usage_internals = (
                Some(crate::service::get_process_cpu_usage_percentage(item.pid as i64)),
                process.memory_info().ok(),
              );
            }
