...
use crate::helpers;
use crate::process::RemoteConfig;
...
pub fn read() -> Config {
  match home::home_dir() {
    Some(path) => {
      ...
      let config_path = format!("{path}/.omnitron/config.toml");
      ...
      if !Exists::check(&config_path).file() {
        let config = Config {
          ...
          runner: Runner {
            ...
            args: vec![string!("-c")],
            node: string!("node"),
            log_path: format!("{path}/.omnitron/logs"),
          },
          daemon: Daemon {
            restarts: 10,
            interval: 1000,
...
pub fn servers() -> Servers {
  match home::home_dir() {
    Some(path) => {
      let path = path.display();
      let config_path = format!("{path}/.omnitron/servers.toml");
...
impl Config {
  ...
  pub fn fmt_address(&self) -> String {
    format!("{}:{}", self.daemon.web.address.clone(), self.daemon.web.port)
  }
