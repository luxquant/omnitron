...
use prometheus::{
  opts, register_counter, register_gauge, register_histogram, register_histogram_vec, Counter, Gauge, Histogram, HistogramVec,
};
...
use crate::webui::assets::NamedFile;
use crate::webui::{self};
use crate::{config, process};

lazy_static! {
  pub static ref HTTP_COUNTER: Counter =
    register_counter!(opts!("http_requests_total", "Number of HTTP requests made.")).unwrap();
  pub static ref DAEMON_START_TIME: Gauge =
    register_gauge!(opts!("process_start_time_seconds", "The uptime of the daemon.")).unwrap();
  pub static ref DAEMON_MEM_USAGE: Histogram =
    register_histogram!("daemon_memory_usage", "The memory usage graph of the daemon.").unwrap();
  pub static ref DAEMON_CPU_PERCENTAGE: Histogram =
    register_histogram!("daemon_cpu_percentage", "The cpu usage graph of the daemon.").unwrap();
...
pub async fn start(webui: bool) {
  ...
  let rocket = rocket::custom(config::read().get_address())
    ...
    .register(
      "/",
      rocket::catchers![internal_error, bad_request, not_allowed, not_found, unauthorized],
    )
    .launch()
    .await;
...
async fn dynamic_assets(name: String) -> Option<NamedFile> {
  static DIR: Dir = include_dir!("omnitron-pm/src/webui/dist/assets");
...
async fn static_assets(name: String) -> Option<NamedFile> {
  static DIR: Dir = include_dir!("omnitron-pm/src/daemon/static");
  let file = DIR.get_file(&name)?;
