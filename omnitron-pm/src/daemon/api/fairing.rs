...
use rocket::{async_trait, Data, Orbit, Request, Response, Rocket};
...
impl Fairing for super::Logger {
  ...
  async fn on_liftoff(&self, rocket: &Rocket<Orbit>) {
    ...
    // daemon_log!("[rocket] limits", "limits" => config.limits);
    // daemon_log!("[api] server started", "port" => config.port, "host" => config.address);
  }
