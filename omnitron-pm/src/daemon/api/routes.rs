...
use super::helpers::{generic_error, not_found, GenericError, NotFound};
use super::structs::ErrorMessage;
use super::{render, EnableWebUI, TeraState};
use crate::daemon::api::{HTTP_COUNTER, HTTP_REQ_HISTOGRAM};
use crate::daemon::pid::{self, Pid};
use crate::process::http::client;
use crate::process::{dump, ItemSingle, ProcessItem, Runner};
use crate::{config, file, helpers};
...
pub(crate) struct ConfigBody {
  ...
  args: Vec<String>,
  #[schema(example = "/home/user/.omnitron/logs")]
  log_path: String,
}
...
pub async fn get_metrics() -> MetricsRoot {
  ...
  let mut runner: Runner = file::read_object(global!("omnitron.dump"));
  ...
  if pid::exists() {
    if let Ok(process_id) = pid::read() {
      if let Ok(process) = Process::new(process_id.get()) {
        ...
        cpu_percent = Some(crate::service::get_process_cpu_usage_percentage(process_id.get::<i64>()));
      }
    }
  }
  ...
  MetricsRoot {
    ...
    daemon: Daemon {
      ...
      daemon_type: global!("omnitron.daemon.kind"),
      stats: Stats {
