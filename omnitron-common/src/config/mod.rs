...
use crate::{ListenEndpoint, OmnitronError, Secret};
...
impl UserRequireCredentialsPolicy {
  ...
  pub fn upgrade_to_otp(&self, with_existing_credentials: &[UserAuthCredential]) -> Self {
    ...
    if let Some(policy) = &mut copy.http {
      ...
      let mut kinds = vec![];
      if with_existing_credentials.iter().any(|c| c.kind() == CredentialKind::Password) {
    ...
    if let Some(policy) = &mut copy.ssh {
      ...
      if with_existing_credentials
        .iter()
        .any(|c| c.kind() == CredentialKind::Password || c.kind() == CredentialKind::PublicKey)
      {
        kinds.push(CredentialKind::Password);
      }
      if !kinds.is_empty() {
...
#[derive(Debug, Clone)]
pub struct OmnitronConfig {
  pub store: OmnitronConfigStore,
...
impl OmnitronConfig {
  pub fn external_host_from_config(&self) -> Option<(Scheme, String, Option<u16>)> {
    if let Some(external_host) = self.store.external_host.as_ref() {
      ...
      Some((
        ...
        self.store.http.external_port.or(Some(self.store.http.listen.port())),
      ))
    } else {
      None
    }
  ...
  pub fn external_host_from_request(&self, request: &poem::Request) -> Option<(Scheme, String, Option<u16>)> {
  ...
  pub fn construct_external_url(
    ...
    Url::parse(&url).map_err(OmnitronError::UrlParse)
  }
