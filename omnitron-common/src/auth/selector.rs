...
pub enum AuthSelector {
  User { username: String, target_name: String },
  Ticket { secret: Secret<String> },
}
...
impl<T: AsRef<str>> From<T> for AuthSelector {
  fn from(selector: T) -> Self {
    ...
    let separator = if selector.as_ref().contains('#') { '#' } else { ':' };
...
impl Debug for AuthSelector {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      AuthSelector::User { username, target_name } => write!(f, "<{username} for {target_name}>"),
