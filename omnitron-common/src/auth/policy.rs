...
pub trait CredentialPolicy {
  fn is_sufficient(&self, protocol: &str, valid_credentials: &[AuthCredential]) -> CredentialPolicyResponse;
}
...
impl CredentialPolicy for AnySingleCredentialPolicy {
  fn is_sufficient(&self, _protocol: &str, valid_credentials: &[AuthCredential]) -> CredentialPolicyResponse {
    if valid_credentials.is_empty() {
      CredentialPolicyResponse::Need(self.supported_credential_types.clone().into_iter().collect())
    } else {
...
impl CredentialPolicy for AllCredentialsPolicy {
  fn is_sufficient(&self, _protocol: &str, valid_credentials: &[AuthCredential]) -> CredentialPolicyResponse {
    let valid_credential_types: HashSet<CredentialKind> = valid_credentials.iter().map(|x| x.kind()).collect();

    if !valid_credential_types.is_empty() && valid_credential_types.is_superset(&self.required_credential_types) {
      CredentialPolicyResponse::Ok
    } else {
