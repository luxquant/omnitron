...
use omnitron_common::helpers::fs::secure_file;
use omnitron_common::{OmnitronConfig, OmnitronConfigStore};
use tokio::sync::{broadcast, mpsc, Mutex};
use tracing::*;

pub fn load_config(path: &Path, secure: bool) -> Result<OmnitronConfig> {
  ...
  let store: OmnitronConfigStore = serde_yaml::from_value(store).context("Could not load config")?;
        ...
        if let Value::Mapping(ref mut user) = user {
          if let Some(new_require) = match user.get(Value::String("require".into())) {
            Some(Value::Sequence(ref old_requires)) => Some(Value::Mapping(
              vec![
                (Value::String("ssh".into()), Value::Sequence(old_requires.clone())),
                (Value::String("http".into()), Value::Sequence(old_requires.clone())),
              ]
              .into_iter()
              .collect(),
            )),
...
pub fn watch_config<P: AsRef<Path> + Send + 'static>(
  path: P,
  config: Arc<Mutex<OmnitronConfig>>,
