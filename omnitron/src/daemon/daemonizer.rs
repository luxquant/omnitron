...
use nix::fcntl::{flock, open, OFlag};
use nix::libc::{getgrgid, getgrnam, getpwnam, getpwuid, mode_t, STDERR_FILENO, STDIN_FILENO, STDOUT_FILENO};
use nix::sys::stat::{umask, Mode};
use nix::unistd::{close, dup, fork, geteuid, getpid, setgid, setsid, setuid, write, Gid, Uid};
...
impl Daemonizr {
  ...
  pub fn spawn(mut self) -> Result<(), DaemonizrError> {
    ...
    let stdi = match open(
      ...
      Err(e) => return Err(DaemonizrError::FailedToReopen("stdin".to_owned(), e.to_string())),
      Ok(x) => x,
    };
    ...
    if let Err(e) = stdo {
      return Err(DaemonizrError::FailedToReopen("stdout".to_owned(), e.to_string()));
    }
    ...
    if let Err(e) = stde {
      return Err(DaemonizrError::FailedToReopen("stderr".to_owned(), e.to_string()));
    }
    ...
    match flock(self.fd_lock, nix::fcntl::FlockArg::LockExclusiveNonblock) {
      ...
      Ok(_) => {
        let pid = getpid();
        let pidb = format!("{}\n", pid.as_raw());
        if let Err(e) = unsafe { write(BorrowedFd::borrow_raw(self.fd_lock), pidb.as_bytes()) } {
        ...
        match nix::unistd::read(pf_fd, &mut buf as &mut [u8]) {
          ...
          Ok(u) => {
            let s = String::from_utf8(buf.to_vec())
              ...
              .to_string();
            if u > 0 {
              (pf_fd, u32::from_str_radix(&s, 10).expect("unable to parse PID to number"))
            } else {
              return Err(DaemonizrError::FailedToReadPidfile(format!("invalid pid: {}", s).to_owned()));
            }
          }
        }
      }
