...
use omnitron_gate_core::logging::{make_database_logger_layer, make_socket_logger_layer};
...
pub async fn init_logging(config: Option<&OmnitronConfig>, cli: &Cli) {
  if std::env::var("RUST_LOG").is_err() {
    match cli.debug {
      0 => std::env::set_var("RUST_LOG", "omnitron=info"),
      1 => std::env::set_var("RUST_LOG", "omnitron=debug"),
      2 => std::env::set_var("RUST_LOG", "omnitron=debug,russh=debug"),
