...
impl Encode<'_, ()> for Handshake {
  fn encode_with(&self, buf: &mut Vec<u8>, _: ()) {
    ...
    if self.server_capabilities.contains(Capabilities::SECURE_CONNECTION) {
...
fn test_decode_handshake_mysql_8_0_18() {
  ...
  assert!(matches!(p.auth_plugin, Some(AuthPlugin::CachingSha2Password)));
...
fn test_decode_handshake_mariadb_10_4_7() {
  ...
  assert!(p.status.contains(Status::SERVER_STATUS_AUTOCOMMIT));
  assert!(matches!(p.auth_plugin, Some(AuthPlugin::MySqlNativePassword)));
