...
impl Decode<'_, Capabilities> for ErrPacket {
  fn decode_with(mut buf: Bytes, capabilities: Capabilities) -> Result<Self, Error> {
    ...
    if header != 0xff {
      return Err(err_protocol!("expected 0xff (ERR_Packet) but found 0x{:x}", header));
    }
...
fn test_decode_err_packet_out_of_order() {
  ...
  let p = ErrPacket::decode_with(ERR_PACKETS_OUT_OF_ORDER.into(), Capabilities::PROTOCOL_41).unwrap();
...
fn test_decode_err_packet_unknown_database() {
  ...
  let p = ErrPacket::decode_with(ERR_HANDSHAKE_UNKNOWN_DB.into(), Capabilities::PROTOCOL_41).unwrap();
