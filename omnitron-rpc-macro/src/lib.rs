...
use syn::token::Comma;
use syn::{
  braced, parenthesized, parse_macro_input, parse_quote, AttrStyle, Attribute, Expr, FnArg, Ident, Lit, LitBool, MetaNameValue,
    ...
    for rpc in &rpcs {
      if rpc.ident == "new" {
        extend_errors!(
          ...
          syn::Error::new(
            rpc.ident.span(),
            format!("method name conflicts with generated fn `{}Client::new`", ident.unraw())
          )
        );
      }
      if rpc.ident == "serve" {
  ...
  fn parse(input: ParseStream) -> syn::Result<Self> {
    ...
    for meta in meta_items {
      if meta.path.segments.len() != 1 {
        extend_errors!(
          result,
          syn::Error::new(meta.span(), "omnitron_rpc::service does not support this meta item")
        );
        continue;
      }
      ...
      if segment.ident == "derive" {
        ...
        match expr_lit.lit {
          ...
          _ => extend_errors!(
            result,
            syn::Error::new(expr_lit.lit.span(), "`derive_serde` expects a value of type `bool`")
          ),
        }
    ...
    if has_derive_serde {
      let deprecation_hack = quote! {
          const _: () = {
              #[deprecated(
                  note = "\nThe form `omnitron_rpc::service(derive_serde = true)` is deprecated.\
                  \nUse `omnitron_rpc::service(derive = [Serialize, Deserialize])`."
              )]
    ...
    if has_explicit_derives & has_derive_serde {
      extend_errors!(
        ...
        syn::Error::new(
          input.span(),
          "omnitron_rpc does not support `derive_serde` and `derive` at the same time"
        )
      );
    }
    ...
    if derive_serde.len() > 1 {
      for (i, derive_serde) in derive_serde.iter().enumerate() {
        extend_errors!(
          ...
          syn::Error::new(
            derive_serde.span(),
            format!("`derive_serde` appears more than once (occurrence #{})", i + 1)
          )
        );
      }
    }
...
///
/// ```rust
/// #[derive(::omnitron_rpc::serde::Serialize, ::omnitron_rpc::serde::Deserialize)]
/// #[serde(crate = "omnitron_rpc::serde")]
/// # struct Foo;
/// ```
...
///
/// ```no_run
/// use omnitron_rpc::{client, transport, service, server::{self, Channel}, context::Context};
///
/// #[service]
...
impl<'a> ServiceGenerator<'a> {
  fn trait_service(&self) -> TokenStream2 {
    ...
    let rpc_fns = rpcs
      ...
      .map(|(RpcMethod { attrs, ident, args, .. }, output)| {
        quote! {
            #( #attrs )*
            async fn #ident(self, context: ::omnitron_rpc::context::Context, #( #args ),*) -> #output;
        }
      });
    ...
    quote! {
        ...
        #vis trait #service_ident: ::core::marker::Sized {
            ...
            /// [InFlightRequest::execute](::omnitron_rpc::server::InFlightRequest::execute).
        ...
        #[doc = #stub_doc]
        #vis trait #client_stub_ident: ::omnitron_rpc::client::stub::Stub<Req = #request_ident, Resp = #response_ident> {
        }

        impl<S> #client_stub_ident for S
            where S: ::omnitron_rpc::client::stub::Stub<Req = #request_ident, Resp = #response_ident>
        {
        }
    }
  }
  ...
  fn struct_server(&self) -> TokenStream2 {
    ...
    quote! {
        /// A serving function to use with [::omnitron_rpc::server::InFlightRequest::execute].
        #[derive(Clone)]
  ...
  fn impl_serve_for_server(&self) -> TokenStream2 {
    ...
    quote! {
        impl<S> ::omnitron_rpc::server::Serve for #server_ident<S>
            where S: #service_ident
        {
            ...
            async fn serve(self, ctx: ::omnitron_rpc::context::Context, req: #request_ident)
                -> ::core::result::Result<#response_ident, ::omnitron_rpc::ServerError> {
                match req {
                    #(
                        #( #method_cfgs )*
  ...
  fn enum_request(&self) -> TokenStream2 {
    ...
    quote! {
        ...
        impl ::omnitron_rpc::RequestName for #request_ident {
            fn name(&self) -> &str {
                match self {
                    #(
                        #( #method_cfgs )*
  ...
  fn struct_client(&self) -> TokenStream2 {
    ...
    quote! {
        ...
        #vis struct #client_ident<
            Stub = ::omnitron_rpc::client::Channel<#request_ident, #response_ident>
        >(Stub);
    }
  }
  ...
  fn impl_client_new(&self) -> TokenStream2 {
    ...
    quote! {
        impl #client_ident {
            ...
            #vis fn new<T>(config: ::omnitron_rpc::client::Config, transport: T)
                -> ::omnitron_rpc::client::NewClient<
                    Self,
                    ::omnitron_rpc::client::RequestDispatch<#request_ident, #response_ident, T>
                >
            where
                T: ::omnitron_rpc::Transport<::omnitron_rpc::ClientMessage<#request_ident>, ::omnitron_rpc::Response<#response_ident>>
            {
                let new_client = ::omnitron_rpc::client::new(config, transport);
                ::omnitron_rpc::client::NewClient {
        ...
        impl<Stub> ::core::convert::From<Stub> for #client_ident<Stub>
            where Stub: ::omnitron_rpc::client::stub::Stub<
                Req = #request_ident,
                Resp = #response_ident>
        {
            /// Returns a new client stub that sends requests over the given transport.
    ...
    quote! {
                ...
                Resp = #response_ident>
        {
            #(
                #[allow(unused)]
                #( #method_attrs )*
                #vis fn #method_idents(&self, ctx: ::omnitron_rpc::context::Context, #( #args ),*)
                    -> impl ::core::future::Future<Output = ::core::result::Result<#return_types, ::omnitron_rpc::client::RpcError>> + '_ {
