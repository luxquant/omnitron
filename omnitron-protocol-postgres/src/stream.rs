...
use bytes::BytesMut;
use omnitron_common::{MaybeTlsStream, MaybeTlsStreamError, UpgradableStream};
  ...
  TS: AsyncRead + AsyncWrite + Unpin,
{
  ...
  pub fn push<M: PostgresEncode + Debug>(&mut self, message: M) -> Result<(), PostgresStreamError> {
  ...
  pub(crate) async fn recv<T: PostgresDecode + Debug>(&mut self) -> Result<Option<T>, PostgresStreamError> {
    loop {
