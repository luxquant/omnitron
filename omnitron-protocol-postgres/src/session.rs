...
use omnitron_common::{Secret, TargetOptions, TargetPostgresOptions};
...
impl PostgresSession {
  ...
  pub async fn run(mut self) -> Result<(), PostgresError> {
    let Some(mut initial_message) = self.stream.recv::<PgWireStartupOrSslRequest>().await? else {
      return Err(PostgresError::Eof);
    };

    if let PgWireStartupOrSslRequest::SslRequest(_) = &initial_message {
      debug!("Received SslRequest");
      self.stream.push(pgwire::messages::response::SslResponse::Accept)?;
    ...
    let password = if let AuthSelector::Ticket { .. } = AuthSelector::from(username.as_deref().unwrap_or("")) {
      ...
      let Some(PgWireGenericFrontendMessage(PgWireFrontendMessage::PasswordMessageFamily(message))) =
        self.stream.recv::<PgWireGenericFrontendMessage>().await?
      else {
        return Err(PostgresError::Eof);
      };

      Secret::from(message.into_password().map_err(PostgresError::from)?.password)
    };

    self
      .run_authorization(startup, &username.unwrap_or("".into()), password)
      .await
  }
  ...
  pub async fn run_authorization(
    ...
    async fn fail(this: &mut PostgresSession) -> Result<(), PostgresError> {
      let error_info = ErrorInfo::new("FATAL".to_owned(), "28P01".to_owned(), "Authentication failed".to_owned());
      ...
      AuthSelector::Ticket { secret } => {
          ...
          .map_err(PostgresError::other)?
        {
          Some(ticket) => {
            ...
            self.run_authorized(startup, ticket.username, ticket.target).await
          }
  ...
  async fn run_authorized(
    ...
    self.stream.push(pgwire::messages::startup::Authentication::Ok)?;
    self.stream.flush().await?;
    ...
    let Some((target, postgres_options)) = target else {
      ...
      self
        .send_error_response("0W001".into(), format!("Omnitron target {target_name} not found"))
        .await?;
      return Ok(());
    };
  ...
  async fn send_error_response(&mut self, code: String, message: String) -> Result<(), PostgresError> {
  ...
  async fn run_authorized_inner(
    ...
    .await
    {
      Err(error) => {
        self
          .send_error_response("0W002".into(), "Omnitron target connection failed".into())
          .await?;
        Err(error)
      }
      x => x,
