...
use omnitron_common::helpers::hash::generate_ticket_secret;
...
impl Api {
  ...
  ) -> Result<GetApiTokensResponse, OmnitronError> {
  ...
  ) -> Result<CreateApiTokenResponse, OmnitronError> {
    let db = services.db.lock().await;
  ...
  async fn api_delete_api_token(
    ...
    services: Data<&Services>,
    id: Path<Uuid>,
  ) -> Result<DeleteApiTokenResponse, OmnitronError> {
    let db = services.db.lock().await;
