...
use tracing::*;
use omnitron_common::auth::{AuthCredential, AuthResult};
use omnitron_common::OmnitronError;
use omnitron_core::Services;
use omnitron_sso::{SsoClient, SsoInternalProviderConfig};
...
fn make_redirect_url(err: &str) -> String {
    error!("SSO error: {err}");
    format!("/@omnitron?login_error={err}")
}
...
impl Api {
    ...
    ) -> Result<GetSsoProvidersResponse, OmnitronError> {
    ...
    ) -> Result<Response<ReturnToSsoResponse>, OmnitronError> {
        let url = self
    ...
    ) -> Result<ReturnToSsoPostResponse, OmnitronError> {
        let url = self
    ...
    async fn api_return_to_sso_get_common(
        ...
        services: Data<&Services>,
        code: &Option<String>,
    ) -> Result<Result<String, String>, OmnitronError> {
    ...
    ) -> Result<StartSloResponse, OmnitronError> {
