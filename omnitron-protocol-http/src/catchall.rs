...
use omnitron_common::{Target, TargetHTTPOptions, TargetOptions};
use omnitron_core::{OmnitronServerHandle, Services};
...
#[derive(Deserialize)]
struct QueryParams {
  #[serde(rename = "omnitron-target")]
  omnitron_target: Option<String>,
}

pub fn target_select_redirect() -> Response {
  Redirect::temporary("/@omnitron").into_response()
}
...
pub async fn catchall_endpoint(
  ...
  server_handle: Option<Data<&Arc<Mutex<OmnitronServerHandle>>>>,
...
async fn get_target_for_request(req: &Request, services: &Services) -> poem::Result<Option<(Target, TargetHTTPOptions)>> {
  ...
  let username = match *auth {
    RequestAuthorization::Session(SessionAuthorization::Ticket { target_name, username }) => {
    ...
    RequestAuthorization::Session(SessionAuthorization::User(username)) => {
      ...
      selected_target_name = host_based_target_name.or(if let Some(omnitron_target) = params.omnitron_target {
        Some(omnitron_target)
      } else {
        session.get_target_name()
      });
      username
    }
    RequestAuthorization::UserToken { .. } | RequestAuthorization::AdminToken => return Ok(None),
  };
