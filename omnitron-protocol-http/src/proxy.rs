...
use tracing::*;
use url::Url;
use omnitron_common::{try_block, TargetHTTPOptions, TlsMode, OmnitronError};
...
static X_OMNITRON_AUTHENTICATION_TYPE: HeaderName =
    HeaderName::from_static("x-omnitron-authentication-type");
...
async fn inject_own_headers<B: SomeRequestBuilder>(req: &Request, mut target: B) -> Result<B> {
    ...
    if let Some(auth) = session.get_auth() {
        target = target
            .header(&X_OMNITRON_USERNAME, auth.username().into())
            .header(
...
pub async fn proxy_normal_request(
    ...
    client_request = client_request.header(
        http::header::HOST,
        uri.authority()
            .ok_or(OmnitronError::NoHostInUrl)?
            .to_string(),
    );
...
async fn copy_client_body_and_embed(
    ...
    let mut inject = format!(
        r#"<script type="module" src="/@omnitron/{}"></script>"#,
