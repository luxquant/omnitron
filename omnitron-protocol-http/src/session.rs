...
use omnitron_core::{Services, SessionStateInit, OmnitronServerHandle};

use crate::common::PROTOCOL_NAME;
use crate::session_handle::{
    HttpSessionHandle, SessionHandleCommand, OmnitronServerHandleFromRequest,
};
...
impl SessionStore {
    ...
    pub async fn create_handle_for(
        &mut self,
        req: &Request,
    ) -> poem::Result<OmnitronServerHandleFromRequest> {
        let session = <&Session>::from_request_without_body(req).await?;
