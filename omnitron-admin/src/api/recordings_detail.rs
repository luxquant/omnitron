...
use omnitron_core::recordings::{AsciiCast, SessionRecordings, TerminalRecordingItem};
use omnitron_db_entities::Recording::{self, RecordingKind};
...
#[OpenApi]
impl Api {
  #[oai(path = "/recordings/:id", method = "get", operation_id = "get_recording")]
...
pub async fn api_get_recording_cast(
  ...
  let path = { recordings.lock().await.path_for(&recording.session_id, &recording.name) };
  ...
  while let Some(line) = lines.next_line().await.map_err(InternalServerError)? {
    let entry: TerminalRecordingItem = serde_json::from_str(&line[..]).map_err(InternalServerError)?;
