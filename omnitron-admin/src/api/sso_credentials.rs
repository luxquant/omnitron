...
use uuid::Uuid;
use omnitron_common::{UserSsoCredential, OmnitronError};
use omnitron_db_entities::SsoCredential;
...
impl ListApi {
    ...
    ) -> Result<GetSsoCredentialsResponse, OmnitronError> {
    ...
    async fn api_create(
        ...
        user_id: Path<Uuid>,
        _auth: AnySecurityScheme,
    ) -> Result<CreateSsoCredentialResponse, OmnitronError> {
        let db = db.lock().await;
...
impl DetailApi {
    ...
    async fn api_update(
        ...
        id: Path<Uuid>,
        _auth: AnySecurityScheme,
    ) -> Result<UpdateSsoCredentialResponse, OmnitronError> {
        let db = db.lock().await;
