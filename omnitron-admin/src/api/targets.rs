...
use uuid::Uuid;
use omnitron_common::{Role as RoleConfig, Target as TargetConfig, TargetOptions, OmnitronError};
...
impl DetailApi {
    ...
    async fn api_update_target(
        ...
        model.options =
            Set(serde_json::to_value(body.options.clone()).map_err(OmnitronError::from)?);
