...
use omnitron_common::{OmnitronError, Role as RoleConfig};
use omnitron_core::consts::BUILTIN_ADMIN_ROLE_NAME;
use omnitron_db_entities::Role;
use poem::web::Data;
...
impl ListApi {
  ...
  async fn api_get_all_roles(
  ...
  ) -> Result<GetRolesResponse, OmnitronError> {
    ...
    Ok(GetRolesResponse::Ok(Json(roles.into_iter().map(Into::into).collect())))
  }
  ...
  async fn api_create_role(
    ...
    let role = values.insert(&*db).await.map_err(OmnitronError::from)?;
...
impl DetailApi {
  ...
  async fn api_get_role(
    ...
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<GetRoleResponse, OmnitronError> {
    let db = db.lock().await;
  ...
  async fn api_update_role(
    ...
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<UpdateRoleResponse, OmnitronError> {
    let db = db.lock().await;
  ...
  async fn api_delete_role(
    ...
    id: Path<Uuid>,
    _auth: AnySecurityScheme,
  ) -> Result<DeleteRoleResponse, OmnitronError> {
    let db = db.lock().await;
