...
static LOG_SENDER: OnceCell<tokio::sync::broadcast::Sender<LogEntry::ActiveModel>> = OnceCell::new();
  ...
  S: Subscriber + for<'a> LookupSpan<'a>,
{
  ...
  ValuesLogLayer::new(|values| {
    println!("LOG_SENDER: {:?}", LOG_SENDER.get());
...
pub fn install_database_logger(database: Arc<Mutex<DatabaseConnection>>) {
  tokio::spawn(async move {
    ...
    let mut receiver = LOG_SENDER.get().expect("Log sender not ready yet").subscribe();
    loop {
