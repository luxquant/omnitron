...
use omnitron_gate_common::auth::{AuthResult, AuthState, CredentialKind};
use omnitron_gate_common::{OmnitronError, SessionId};
...
impl AuthStateStore {
  ...
  pub async fn create(
  ...
  ) -> Result<(Uuid, Arc<Mutex<AuthState>>), OmnitronError> {
    let id = Uuid::new_v4();
    let policy = self
      .config_provider
      .lock()
    ...
    let Some(policy) = policy else {
      return Err(OmnitronError::UserNotFound(username.into()));
    };

    let state = AuthState::new(id, session_id.copied(), username.to_string(), protocol.to_string(), policy);
    self.store.insert(id, (Arc::new(Mutex::new(state)), Instant::now()));
  ...
  pub async fn vacuum(&mut self) {
    self.store.retain(|_, (_, started_at)| started_at.elapsed() < *TIMEOUT);

    self.completion_signals.retain(|_, signal| !signal.is_expired());
  }
}
