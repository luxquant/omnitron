...
#[tokio::test]
async fn sequential() {
  let (tx, rx) = omnitron_rpc::transport::channel::unbounded();
  let client = client::new(client::Config::default(), tx).spawn();
  let channel = BaseChannel::with_defaults(rx);
  tokio::spawn(
    channel
      .execute(omnitron_rpc::server::serve(|_, i: u32| async move { Ok(i + 1) }))
...
async fn serde_tcp() -> anyhow::Result<()> {
  use omnitron_rpc::serde_transport;
  use tokio_serde::formats::Json;

  let _ = tracing_subscriber::fmt::try_init();

  let transport = omnitron_rpc::serde_transport::tcp::listen("localhost:56789", Json::default).await?;
...
async fn serde_uds() -> anyhow::Result<()> {
  ...
  let sock = omnitron_rpc::serde_transport::unix::TempPathBuf::with_random("uds");
  let transport = omnitron_rpc::serde_transport::unix::listen(&sock, Json::default).await?;
  tokio::spawn(
    transport
      .take(1)
      .filter_map(|r| async { r.ok() })
