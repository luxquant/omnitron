  ...
  T: Transport<Response<Resp>, ClientMessage<Req>>,
{
  ...
  fn start_request(mut self: Pin<&mut Self>, mut request: Request<Req>) -> Result<TrackedRequest<Req>, AlreadyExistsError> {
    ...
    let start = self
      ...
      .start_request(request.id, request.context.deadline, span.clone());
    match start {
  ...
  Self: Transport<Response<<Self as Channel>::Resp>, TrackedRequest<<Self as Channel>::Req>>,
{
  ...
  ///
  /// ```rust
  /// use omnitron_rpc::{context, client, server::{self, BaseChannel, Channel, serve}, transport};
  /// use futures::prelude::*;
...
mod tests {
  ...
  async fn serve_before_error_aborts_request() -> anyhow::Result<()> {
    ...
    let deadline_hook =
      serve.before(|_: &mut context::Context, _: &i32| async { Err(ServerError::new(io::ErrorKind::Other, "oops".into())) });
