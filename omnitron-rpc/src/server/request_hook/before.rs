...
pub trait BeforeRequestList<Req>: BeforeRequest<Req> {
  ...
  fn then_fn<Next: FnMut(&mut context::Context, &Req) -> Fut, Fut: Future<Output = Result<(), ServerError>>>(
    self,
    next: Next,
  ) -> Self::Then<Next>
  where
...
/// use omnitron_rpc::{context, ServerError, server::{Serve, serve, request_hook::{self,
