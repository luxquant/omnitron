...
where
  ...
  TS: AsyncRead + AsyncWrite + Unpin,
{
  pub async fn upgrade(mut self, tls_config: S::UpgradeConfig) -> Result<Self, MaybeTlsStreamError> {
...
where
  ...
  TS: AsyncRead + AsyncWrite + Unpin,
{
  fn poll_read(self: Pin<&mut Self>, cx: &mut std::task::Context<'_>, buf: &mut ReadBuf<'_>) -> Poll<tokio::io::Result<()>> {
    match self.get_mut() {
  ...
  S: AsyncRead + AsyncWrite + Unpin + Send,
{
  ...
  async fn upgrade(mut self, config: Self::UpgradeConfig) -> Result<tokio_rustls::client::TlsStream<S>, MaybeTlsStreamError> {
  ...
  S: AsyncRead + AsyncWrite + Unpin + Send,
{
  ...
  async fn upgrade(mut self, tls_config: Self::UpgradeConfig) -> Result<tokio_rustls::server::TlsStream<S>, MaybeTlsStreamError> {
