...
use omnitron_common::auth::{AuthCredential, AuthResult, AuthSelector, CredentialKind};
use omnitron_common::helpers::rng::get_crypto_rng;
use omnitron_common::{Secret, TargetMySqlOptions, TargetOptions};
use omnitron_core::{authorize_ticket, consume_ticket, OmnitronServerHandle, Services};
use omnitron_database_protocols::io::{BufExt, Decode};
use omnitron_database_protocols::mysql::protocol::auth::AuthPlugin;
use omnitron_database_protocols::mysql::protocol::connect::{AuthSwitchRequest, Handshake, HandshakeResponse};
use omnitron_database_protocols::mysql::protocol::response::{ErrPacket, OkPacket, Status};
use omnitron_database_protocols::mysql::protocol::text::Query;
use omnitron_database_protocols::mysql::protocol::Capabilities;
use rand::Rng;
...
impl MySqlSession {
  ...
  pub async fn run(mut self) -> Result<(), MySqlError> {
    ...
    let handshake = Handshake {
      protocol_version: 10,
      server_version: "8.0.0-Omnitron".to_owned(),
      connection_id: 1,
    ...
    let resp = loop {
      ...
      if self.capabilities.contains(Capabilities::SSL) {
        ...
        self.send_error(1002, "Omnitron requires TLS - please enable it in your client: add `--ssl` on the CLI or add `?sslMode=PREFERRED` to your database URI").await?;
  ...
  pub async fn run_authorization(mut self, handshake: HandshakeResponse, password: Secret<String>) -> Result<(), MySqlError> {
    ...
    async fn fail(this: &mut MySqlSession) -> Result<(), MySqlError> {
      this.stream.push(
        &ErrPacket {
          error_code: 1,
          error_message: "Omnitron access denied".to_owned(),
          sql_state: None,
        },
        (),
      )?;
      ...
      AuthSelector::Ticket { secret } => {
          ...
          .map_err(MySqlError::other)?
        {
          Some(ticket) => {
            ...
            self.run_authorized(handshake, ticket.username, ticket.target).await
          }
  ...
  async fn run_authorized_inner(mut self, handshake: HandshakeResponse, options: TargetMySqlOptions) -> Result<(), MySqlError> {
